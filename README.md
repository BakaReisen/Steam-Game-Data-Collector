...
本项目是一个 **全栈 Steam 游戏数据采集系统**,集成了数据采集、清洗、机器学习和可视化功能。专为商务数据分析课程设计,实现了从数据采集到分析的完整工作流。

### 🎯 项目目标

- **数据采集**: 从 Steam、SteamSpy、SteamCharts 等多个平台采集游戏数据
- **评论分析**: 收集和分析游戏评论,理解玩家情感和偏好
- **数据清洗**: 智能识别缺失值,结合 API 和机器学习模型自动补全
- **预测建模**: 训练机器学习模型,预测游戏在线人数峰值
- **可视化界面**: 提供 Web 界面实时监控和管理数据采集流程

### 🌟 项目特色

1. **全栈架构**: Angular 前端 + Flask 后端,完整的 Web 应用
2. **实时监控**: WebSocket 实时推送任务进度和日志
3. **多源数据整合**: 集成 4+ 个数据源,确保数据完整性
4. **智能数据补全**: API + ML + 规则三层补全策略
5. **端到端自动化**: 从采集到分析,全流程自动化
...
### 优化了快速启动脚本
现在运行start-fullstack.bat后，若前端与后端都成功启动，则自动打开前端页面

## v2.2 - 后端api

### 上一个版本只做了前端，这次更新新增了后端 API
...
2. **游戏数据采集 API** (`/api/collect/*`)
   - 5 种采集模式: sample, custom, chinese_reviews, steamspy, top_games
   - 实时进度推送
   - 支持任务取消和结果下载

3. **评论采集 API** (`/api/reviews/*`)
   - 支持语言过滤
   - 好评/差评筛选
   - 分页采集机制

4. **模型训练 API** (`/api/train/*`)
   - 3 个 RandomForest 模型训练
   - 训练统计和性能指标
   - 模型文件下载

5. **数据清洗 API** (`/api/clean/*`)
   - API + ML + 规则三层补全
   - 缺失值分析
   - 清洗进度实时跟踪
...
- 更新 `steam-api.service.ts` 使用真实 HTTP 请求
- 移除所有 `setTimeout()` 模拟
- 统一接口定义 (`TaskStatus`, `TaskResponse`)
- 支持文件下载

### 启动脚本

- `start-backend.bat/ps1`: 后端快速启动
- `start-fullstack.bat/ps1`: 全栈一键启动
- 自动依赖检查和安装
...
本次更新优化了前端的快速启动脚本,修复了其无法正常显示中文的问题

## v2.0 - Angular 前端界面

### 🎉 全新 Angular 前端界面

本次更新为项目添加了完整的 **Angular 19 Web 前端**,实现了所有核心功能的可视化操作界面!

#### 新增功能模块

1. **📊 游戏数据采集可视化** (`/data-collection`)
   - 5 种采集模式图形化选择
   - 实时进度监控和日志显示
   - 采集统计数据可视化展示
   - 支持断点续传和结果下载

2. **💬 评论数据采集界面** (`/review-collection`)
   - 可视化配置游戏信息 (AppID、名称)
   - 语言过滤选择 (简体中文/英语/繁体中文/全部)
   - 好评/差评数量独立设置
   - 实时显示采集进度和正负评率

3. **🤖 模型训练控制台** (`/model-training`)
   - 数据集文件选择器
   - 3 个模型独立训练开关 (当前在线/24h峰值/历史峰值)
   - 超参数可视化调节 (测试集比例、树数量、最大深度、随机种子)
   - 训练进度实时跟踪
   - 模型性能指标展示 (MAE、RMSE、R²)
   - 特征重要性可视化

4. **🧹 数据清洗工作台** (`/data-cleaning`)
   - 输入/输出文件配置
   - 清洗选项卡片化选择 (API补全/ML预测/删除无效/生成报告)
   - 批处理进度监控
   - 多方法统计追踪 (API修复/ML预测/简单填充)
   - 清洗速度和时间统计

#### 技术亮点

- ⚡ **Angular 19**: 最新版本,采用 Standalone Components 架构
- 🎨 **统一设计语言**: 紫色渐变主题 (#667eea → #764ba2)
- 📱 **响应式布局**: 完美支持桌面和移动设备
- 🔄 **实时通信**: WebSocket 实时推送任务进度
- 🧩 **模块化架构**: 组件化设计,易于扩展和维护
...
访问 `http://localhost:4200/` 即可使用可视化界面!

#### 下一步计划

- 🔌 **后端 API 集成**: 将模拟替换为真实的 Python 后端调用
- 📈 **数据可视化增强**: 添加图表展示采集和分析结果
- 💾 **文件下载功能**: 直接从界面下载 CSV、模型文件等
- 🔐 **用户认证系统**: 多用户支持和任务管理
